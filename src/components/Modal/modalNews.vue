...
<template>
  <div
    ...
    <div class="content w-2/4 pt-2 justify-center overflow-hidden transition-all ease-in-out duration-1000"
      :class="[modal.animar ? 'bg-opacity-70' : 'opacity-0']">
      <div class="bg-white relative rounded-lg">
        <!-- condición para mostrar si es actualizar   -->
        <h1 class="text-center p-4 text-2xl text-white bg-red-700 rounded-lg mb-5" v-if="objectNew.id">
          Atualizar Noticia
        </h1>
        <h1 class="text-center p-4 text-2xl text-white bg-red-700 rounded-lg mb-5" v-else>
          Nueva Noticia
        </h1>
        <!-- Finaliza ña condición -->
        <h1 class="absolute -top-1 -right-1 h-7 w-7 text-center text-xl text-white rounded-full cursor-pointer"
        ...
        <form class="block px-9 pb-5" @submit.prevent="addNew()">
          <div class="sm:col-span-4">
            <div class="w-full px-0.5">
              <label for="title_news" class="block text-sm font-medium leading-6 text-gray-900">Título destacado</label>
              <div class="mt-2">
                <input id="title_news" name="title_news"
          ...
          <div class="sm:col-span-4">
            <label for="info" class="block text-sm font-medium leading-6 text-gray-900">Descripción</label>
            <div class="mt-2">
              <textarea id="info" name="info" rows="6" cols="50"
                class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-red-500"
                v-model="objectNew.info"></textarea>
            </div>
          </div>
          <div class="flex sm:col-span-4">
            <div class="w-1/2 px-0.5">
              <label for="default_size" class="block text-sm font-medium leading-6 text-gray-900">Imagen</label>
              <div class="mt-2">
                <input
                  ...
                  id="default_size" type="file" @change="handleFileChange" accept="image/*" />
                ...
                <p v-else class="mt-1 text-sm text-gray-500" id="file_input_help">
                  PNG, JPG (MAX. 800x400px).
                </p>
              </div>
            </div>
            <div class="w-1/2 px-0.5">
              <label for="video_name" class="block text-sm font-medium leading-6 text-gray-900">Link de video</label>
              <div class="mt-2">
                <input id="video_name" name="video_name"
                  class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-red-500"
          ...
          <div class="mt-5 mb-5">
            <button type="submit"
              ...
              {{ objectNew.id ? "Actualizar" : "Registrar" }}
