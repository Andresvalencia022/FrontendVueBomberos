...
<template>
  <div>
    ...
    >
    <div
  v-for="event in EventStore.arrayEvents.slice(0, 4)"
  :key="event.id"
  class="mx-6 my-4"
>
  <div
    class="w-full p-6 h-auto bg-white border border-gray-200 rounded-xl shadow-lg dark:bg-gray-800 dark:border-gray-700"
  >
    <!-- Título y acciones -->
    <div class="flex items-center justify-between mb-4">
      <h5
        class="w-8/12 text-lg font-bold tracking-wide text-gray-900 dark:text-white truncate"
      >
        {{ event.event_name }}
      </h5>
      ...
      <div class="flex gap-3">
        <a @click="eventUpdate(event.id)">
          <svg
            ...
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6 text-green-500 hover:text-green-700 transition-colors"
          >
            <path
        ...
        <a @click="eventDelete(event.id)">
          <svg
            ...
            stroke-width="1.5"
            stroke="currentColor"
            class="w-6 h-6 text-red-600 hover:text-red-900 transition-colors"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
    ...
    <!-- Fechas del evento -->
    <div class="grid grid-cols-2 gap-4 mb-6 text-gray-700 dark:text-gray-400">
      <div class="text-center">
        <h3 class="text-sm font-semibold">📅 Fecha</h3>
        <p class="text-gray-900 dark:text-white text-sm">{{ event.date }}</p>
      </div>
      <div class="text-center">
        <h3 class="text-sm font-semibold">📅 Fecha fin</h3>
        <p class="text-gray-900 dark:text-white text-sm">{{ event.time }}</p>
      </div>
    </div>

    <!-- Botón de detalles -->
    <button
      class="w-full px-4 py-2 text-white font-medium text-sm rounded-lg bg-gray-900 hover:bg-red-600 transition focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
